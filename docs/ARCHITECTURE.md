...
TazPod v0.3.x is a specialized development enclave designed for **Zero-Trust workflows**. It implements a high-security, volatile execution environment using application-level cryptography and memory isolation.
...
3.  **Application (Container)**: Optimized Docker images containing toolstacks (IDE, AWS, K8s, AI).
...
The core security principle of v0.3.x is the **RAM-Only Decryption** and **Project Isolation**.
...
To ensure a seamless experience with AWS SSO and AI tools without sacrificing security, TazPod uses a **Bridging Logic**:

*   **Enclave Targets**: Persistent config paths (e.g., `~/.aws`) are **Bind-Mounted** to the RAM Enclave.
...
---
*Architecture v0.3.x | Documented by Senior Platform Mentor*
